...
Neste whitepaper, discutimos os fundamentos dos transformers, nos quais todos os LLMs modernos se baseiam. Detalhamos a evolução das várias arquiteturas de modelos LLM e seus componentes. Também vimos as diversas metodologias que você pode usar para treinar e ajustar (fine-tune) modelos de forma eficiente e eficaz. Discutimos brevemente a engenharia de prompt e as técnicas de amostragem que influenciam grandemente a saída de um LLM, e também abordamos possíveis aplicações desta tecnologia. Há uma série de pontos-chave a serem lembrados:

*   **A arquitetura transformer é a base para todos os LLMs modernos.** Ao longo das várias arquiteturas mencionadas neste whitepaper, vemos que é importante não apenas adicionar mais parâmetros ao modelo, mas a composição do conjunto de dados é igualmente importante.
*   **A ordem e as estratégias usadas para o ajuste fino (fine-tuning) são importantes** e podem incluir múltiplas etapas, como Ajuste de Instrução (Instruction Tuning), Ajuste de Segurança (Safety Tuning), etc. O Ajuste Fino Supervisionado (SFT - Supervised Fine Tuning) é importante para capturar a essência de uma tarefa. RLHF (Reinforcement Learning from Human Feedback - Aprendizado por Reforço com Feedback Humano) e, potencialmente, RLAIF (Reinforcement Learning from AI Feedback - Aprendizado por Reforço com Feedback de IA), podem ser usados para deslocar a distribuição da pré-treinagem para uma mais desejada através do poder da função de recompensa, que pode recompensar comportamentos desejáveis e penalizar os indesejáveis.
*   **Tornar a inferência de modelos neurais eficiente é um problema importante e um campo ativo de pesquisa.** Muitos métodos existem para reduzir os custos de serviço (serving) e a latência com impacto mínimo no desempenho do modelo, e alguns métodos de aceleração exatos garantem saídas idênticas do modelo.
*   **Modelos de linguagem grandes podem ser usados para uma variedade de tarefas**, incluindo sumarização, tradução, resposta a perguntas, chat, geração de código e muitas outras. Você pode criar suas próprias tarefas usando os serviços de geração de texto do Vertex e Makersuite, que utilizam os modelos de linguagem mais recentes do Google. Após o modelo ter sido treinado e ajustado, **é importante experimentar com a engenharia de prompts.** Você deve usar a técnica mais apropriada para a tarefa em questão, pois os LLMs podem ser sensíveis aos prompts.
*   Além disso, também é possível **aprimorar o desempenho específico da tarefa ou a criatividade e diversidade ajustando os parâmetros** correspondentes às técnicas de amostragem, como Top-K, Top-P e passos máximos de decodificação (Max decoding steps), para encontrar a combinação ótima de correção, diversidade e criatividade necessária para a tarefa em questão.
